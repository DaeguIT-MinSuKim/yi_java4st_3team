...
public class BookPanel extends JPanel implements KeyListener {
	...
	protected void setItem(BookDto idx) {
	...
	public BookDto getItem() {
		...
		return new BookDto(no, name, aut, pub, pri);
	}
	...
	protected void setBookDto(BookDto bdt) {
		BookManagement.pBPanel.setItem(bdt);
	}

	/* 첫번째 필드 회원/도서코드 중복 확인 ↓ */
	public BookDto getCodeNo() {
		// 공란
		if (isEmptyNo()) {
			throw new EmptyTfException("공란 존재");
		}
	...
	public void keyReleased(KeyEvent e) {
		if (tfBno.isEditable()) {// 코드필드 활성화 됐으면
			if (e.getSource() == tfBno) { // 저장할 경우
				String no = tfBno.getText().trim();
				if (no.length() != 0) {
					BookButton.btnAdd.setEnabled(true);
					// System.out.println(no);
				} else {
					BookButton.btnSave.setEnabled(false);
				...
				if (name.length() != 0) {
					...
					BookButton.btnUpdate.setEnabled(false);
				}
				...
				if (bpr.length() != 0) {
					BookButton.btnSave.setEnabled(true);
